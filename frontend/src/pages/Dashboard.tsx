...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-6 sm:space-y-10">
      {/* Welcome Header */}
      <div className="bg-gradient-to-r from-sky-800 to-sky-950 text-white rounded-2xl p-6 sm:p-8 shadow-md flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold mt-2">Welcome, {user?.name || 'Student'}</h1>
          <p className="text-sky-100 text-xs sm:text-sm mt-1">Track your daily assessments and historical performance metrics.</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-3.5 sm:p-4 text-center border border-white/10">
      ...
      {todaysTest && (
        <section className="bg-white rounded-2xl border-2 border-sky-500 shadow-xs p-4 sm:p-8 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 pb-3">
            ...
            <span className="text-xs text-gray-500">Available: {todaysTest.test_date || 'Today'}</span>
          </div>

          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <span className="text-xs font-semibold text-sky-600 uppercase">{todaysTest.exam_name}</span>
              <h2 className="text-xl sm:text-2xl font-extrabold text-gray-900">{todaysTest.title}</h2>
              <p className="text-xs sm:text-sm text-gray-600 mt-1 max-w-xl">{todaysTest.description}</p>
            </div>

            <div className="grid grid-cols-3 gap-3 bg-gray-50 p-3 sm:p-4 rounded-xl border border-gray-200 w-full sm:w-auto text-xs">
              ...
              <div className="text-center">
                ...
                <strong className="text-gray-900">{todaysTest.duration_minutes}m</strong>
              </div>
              <div className="text-center">
          ...
          <div className="flex justify-end pt-2">
            <Link
              ...
              className="w-full sm:w-auto bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs sm:text-sm px-6 py-3 rounded-xl shadow-xs flex items-center justify-center gap-2 transition-all"
            >
      ...
      {/* Exam Category Containers */}
      <section className="space-y-4">
        <h2 className="text-lg sm:text-xl font-bold text-gray-900">Exam Performance Containers</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
          {exams.map((exam) => {
            const stats = getExamStats(exam.name);
            const isGroup2 = exam.slug === 'tnpsc-group-2';
            return (
              <div key={exam.id} className="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6 shadow-2xs space-y-4 flex flex-col justify-between hover:border-sky-300 transition-colors">
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <h3 className="text-base sm:text-lg font-bold text-gray-900">{exam.name}</h3>
                  ...
                  <div className="mt-4 space-y-2 bg-gray-50 p-3 rounded-xl text-xs">
                ...
                <Link
                  to={`/exams/${exam.id}`}
                  className="w-full bg-gray-100 hover:bg-sky-50 hover:text-sky-700 text-gray-700 font-semibold text-xs py-2.5 rounded-xl text-center transition-colors block border border-gray-200"
                >
                  View Category & Tests
                </Link>
              </div>
      ...
      <section className="space-y-4">
        <h2 className="text-lg sm:text-xl font-bold text-gray-900">Recent Test Results</h2>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs sm:text-sm">
                <thead className="bg-gray-50 text-gray-600 font-semibold border-b border-gray-200">
                  <tr>
                    <th className="p-3.5 sm:p-4">Test Title</th>
                    <th className="p-3.5 sm:p-4">Exam</th>
                    <th className="p-3.5 sm:p-4">Submitted Date</th>
                    <th className="p-3.5 sm:p-4">Score</th>
                    <th className="p-3.5 sm:p-4">Percentage</th>
                ...
                <tbody className="divide-y divide-gray-100 text-gray-800">
                  {pastResults.map((res) => (
                    <tr key={res.attempt_id} className="hover:bg-sky-50/50 transition-colors">
                      <td className="p-3.5 sm:p-4 font-bold text-gray-900">{res.test_title}</td>
                      <td className="p-3.5 sm:p-4 font-medium text-sky-700 whitespace-nowrap">{res.exam_name}</td>
                      <td className="p-3.5 sm:p-4 text-xs text-gray-500 whitespace-nowrap">
                        {res.submitted_at ? new Date(res.submitted_at).toLocaleDateString() : 'Today'}
                      </td>
                      <td className="p-3.5 sm:p-4 font-bold whitespace-nowrap">{res.score} / {res.total_questions}</td>
